...
import {GridResponsiveControls} from '../shared/responsive-controls';
import './index.scss';
function Edit({attributes,setAttributes,clientId}) {
 ...
 const resize=value=>{if(String(value).trim()==='')return;const count=Number(value);if(!Number.isInteger(count)||count<0||count>100)return;const next=children.slice(0,count);while(next.length<count)next.push(createBlock('zmblocks/column'));replaceInnerBlocks(clientId,next,false);};
 ...
 <GridResponsiveControls values={v} setAttributes={setAttributes}/>
...
registerBlockType(metadata,{edit:Edit,variations:[{name:'four-columns',title:__('ZM Grid','zmblocks'),isDefault:true,attributes:{columns:'1',columnsSmall:'2',columnsMedium:'3',columnsLarge:'4'},innerBlocks:[['zmblocks/column'],['zmblocks/column'],['zmblocks/column'],['zmblocks/column']]}],save:()=> <InnerBlocks.Content/>});
